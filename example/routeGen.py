...
def generate_routefile():
    ...
    with open("cross.rou.xml", "w") as routes:
        ...
        <vType id="typeBus" accel="0.8" decel="4.5" speedFactor="1.5"
        color="1,0,0" sigma="0.5" length="5" minGap="3" maxSpeed="25" guiShape="emergency"
        lcImpatience="1" jmDriveAterRedTime="1000" 
        lcStrategic="100"
       lcCooperative="0.0"
       lcSpeedGain="100"
       lcKeepRight="100"
       ...
       lcAssertive="1"/>
        ...
        for i in range(N):
            ...
            if random.uniform(0,1) <= pDD:
                print >> routes, '    <vehicle id="%i" type="typeBus" route="down-down" depart="%i" />' % (vehNr, i)
                vehNr += 1
                lastVeh = i
